...
LowerColorError = np.array([-5, -30, -30])
UpperColorError = np.array([5, 30, 30])

info_text = f"Seleciona el circulo trasero de la flecha"
info_text2 = ""
...
def _mouseEvent(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if nClick == 1:
            ...
            print("Eje x: ", x)
            print("Eje y: ", y)
            info_text = f"Seleciona el circulo delantero de la flecha"
            ...
            print("Eje x: ", x)
            print("Eje y: ", y)
            info_text = f"Seleciona el circulo del objetivo"
            nClick += 1
        elif nClick == 3:
            ...
            print("Eje y: ", y)
            nClick += 1
            info_text = "Has seleccionado los tres colores"
            info_text2 = "[Has click para reiniciar]"

            with open("params\colores.txt", "w") as file:
                file.write(str(color1_hsv) + "\n")
                file.write(str(color2_hsv) + "\n")
                file.write(str(color3_hsv) + "\n")

            info_text = "Los colores han sido calibrados correctamente"
            info_text2 = "ADIOOS"
            ...
            info_text = "Seleciona el circulo trasero de la flecha"
            info_text2 = ""
            color1_hsv = np.array([0, 0, 0])
            color2_hsv = np.array([0, 0, 0])
            color3_hsv = np.array([0, 0, 0])
